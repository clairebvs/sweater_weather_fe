...
const getForecastData = () => {
  fetch(`${url}/api/v1/forecast?location=${choosedLocation}`)
    ...
    .then(getBackgroundImage(choosedLocation))
}

const getBackgroundImage = (choosedLocation) => {
  fetch(`${url}/api/v1/backgrounds?location=${choosedLocation}`)
  .catch(error => console.error({error}))
  .then(response => response.json())
  .then(info => updateImage(info))
}

const updateImage = (info) => {
  let image = info['data']['attributes']['city_image']['city_url']
...
const setCurrentForecast = () => {
  ...
  $('#actual-high').text('High: ' + Math.floor(forecastData.dailyForecast()[0]['temperatureHigh']) + '°')
  $('#actual-low').text('Low: ' + Math.floor(forecastData.dailyForecast()[0]['temperatureLow']) + '°')
  $('#today-comment').text('Today: ' + forecastData.dailyForecast()[0]['summary'])
  $('#tonight-comment').text('Tonight: ' + forecastData.dailyForecast()[1]['summary'])
}
